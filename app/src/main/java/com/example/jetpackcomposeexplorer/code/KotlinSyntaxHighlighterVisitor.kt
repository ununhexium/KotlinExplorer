...
class KotlinSyntaxHighlighterVisitor : KotlinParserBaseListener() {
  ...
  override fun visitTerminal(node: TerminalNode) {
    when (node.symbol.tokenIndex) {
      KotlinParser.COMMA -> highlights.addHighlight(
          COMMA,
          node.range
      )
      KotlinParser.VAL -> highlights.addHighlight(
          VAL,
          node.range
      )
      KotlinParser.VAR -> highlights.addHighlight(
          VAR,
          node.range
      )
    }
